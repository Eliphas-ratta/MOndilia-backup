...
<div class="container mx-auto py-10">
    ...
    <div class="flex items-start gap-6">
        <img src="<?= $faction['image'] ?>" alt="<?= $faction['name'] ?>" class="w-80 h-80 object-cover rounded-lg">
        <div>
...
<div class="grid grid-cols-5 gap-6 justify-center px-10 mt-6">
    <?php foreach ($guildes as $guilde) : ?>
        <a href="guilde.php?id=<?= $guilde['id'] ?>" class="transform transition duration-300 hover:scale-105">
            <div class="guilde-card bg-neutral-900 p-4 rounded-lg shadow-lg text-center border-neutral-700 w-64">
                <img src="<?= $guilde['image'] ?>" alt="<?= $guilde['name'] ?>" class="w-full h-full object-contain rounded-lg mb-2">
